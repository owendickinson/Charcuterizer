...
class Logger:
    ...
    def loadRunningBatchLog(self):
        ...
        self.runningBatchLog = dict(
            zip(self.dbCursor.column_names, self.dbCursor.fetchone()))

    def heartbeat(self):
        self.loadRunningBatchLog()
        heartbeatQuery = (
            'update logs_for_batches set time_last_alive = %s where id = %s')
        self.dbCursor.execute(
            heartbeatQuery, (datetime.datetime.now(), self.runningBatchLog['id']))
    ...
    def completedStage(self):
        ...
        if self.runningBatchLog['last_stage_completed'] is not None:
            nextStageCompleted = self.runningBatchLog[
                'last_stage_completed'] + 1
        completedStageQuery = (
            'update logs_for_batches set time_last_alive = %s, time_last_stage_completed = %s, last_stage_completed = %s where id = %s')
        self.dbCursor.execute(completedStageQuery, (datetime.datetime.now(
        ), datetime.datetime.now(), nextStageCompleted, self.runningBatchLog['id']))
        self.dbConnection.commit()

    def newBatch(self, comment=''):
        newBatchQuery = (
            'insert into batches (recipe_id, start_date, comments) values (%s, %s, %s)')
        self.dbCursor.execute(
            newBatchQuery, (self.recipeId, datetime.datetime.now(), comment))
        batchId = self.dbCursor.lastrowid
        print('batchId', batchId)
        newBatchLogQuery = (
            'insert into logs_for_batches (batch_id, time_last_alive) values (%s, %s)')
        self.dbCursor.execute(
            newBatchLogQuery, (batchId, datetime.datetime.now()))
