...
class Recipe:
    ...
    def loadInfoForRecipe(self):
        recipeQuery = (
            'select * from recipes left join literature on recipes.literature_id = literature.id where recipes.id=%s')
        if self.dbCursor is not None:
            self.dbCursor.execute(recipeQuery, (self.recipeId,))
            self.recipeInfo = self.makeDictionaryFromRow(
    ...
    def loadInfoForStages(self):
        ...
        # min humidities
        minHumidityQuery = (
            'select min_humidity.value as min_humidity from humidities_for_recipes left join min_humidity on min_humidity.id = humidities_for_recipes.min_humidity_id where humidities_for_recipes.recipe_id = %s;')
        ...
        # max humidities
        minHumidityQuery = (
            'select max_humidity.value as max_humidity from humidities_for_recipes left join max_humidity on max_humidity.id = humidities_for_recipes.max_humidity_id where humidities_for_recipes.recipe_id = %s;')
        self.dbCursor.execute(minHumidityQuery, (self.recipeId,))
        for row in self.dbCursor:
        ...
        # durations
        durationQuery = (
            'select duration.value as duration from durations_for_recipes left join duration on duration.id = durations_for_recipes.duration_id where durations_for_recipes.recipe_id = %s;')
    ...
    def makeScheduleForRecipe(self, startingStage=0):
        ...
        for minTempDict, maxTempDict in zip(self.minimumTemperatures[startingStage:], self.maximumTemperatures[startingStage:]):
            tempRanges.append(
                (minTempDict['min_temp'], maxTempDict['max_temp']))
        humidityRanges = []
        for minHumDict, maxHumDict in zip(self.minimumHumidities[startingStage:], self.maximumHumidities[startingStage:]):
            humidityRanges.append(
                (minHumDict['min_humidity'], maxHumDict['max_humidity']))
        durationValues = []
    ...
    def loadLogForLastBatch(self):
        logQuery = ('select logs_for_batches.*, batches.start_date from batches left join logs_for_batches on logs_for_batches.batch_id = batches.id where batches.recipe_id = %s order by batches.id desc limit 1;')
        self.dbCursor.execute(logQuery, (self.recipeId,))
        self.logForLastBatch = dict(
            zip(self.dbCursor.column_names, self.dbCursor.fetchone()))

    def makeScheduleForInterruptedRecipe(self, batchId=None):
        self.loadLogForBatch(batchId)
        schedule = None
        stageRunningTimeOnFailure = self.logForLastBatch[
            'time_last_alive'] - self.logForLastBatch['start_date']

        if self.logForLastBatch['last_stage_completed'] is not None:
            schedule = self.makeScheduleForRecipe(startingStage=self.logForLastBatch[
                                                  'last_stage_completed'] + 1)
            stageRunningTimeOnFailure = self.logForLastBatch[
                'time_last_alive'] - self.logForLastBatch['time_last_stage_completed']
        else:
            schedule = self.makeScheduleForRecipe()

        for transitionTimeIndex, transitionTime in enumerate(schedule.transitionTimes):
            schedule.transitionTimes[
                transitionTimeIndex] = transitionTime - stageRunningTimeOnFailure
        return schedule


# select min_temp.value as min_temp from temperatures_for_recipes left
# join min_temp on min_temp.id = temperatures_for_recipes.min_temp_id
# where temperatures_for_recipes.recipe_id = 1;
